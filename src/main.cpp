...
enum class TradingMode { SAFE, SHADOW, LIVE };
static TradingMode g_mode = TradingMode::SHADOW;

// Latency Tracking - Institutional Grade
struct LatencyTracker {
    std::unordered_map<std::string, std::chrono::high_resolution_clock::time_point> pendingOrders;
    std::unordered_map<std::string, std::chrono::high_resolution_clock::time_point> pendingPings;
    std::vector<double> samples;
    double last_exec_rtt = 0.0;
    double last_net_rtt = 0.0;
    double p50 = 0.0;
    double p95 = 0.0;
    bool spike = false;
    bool halted = false;
    const double spike_threshold_ms = 25.0;
    const double hard_kill_ms = 80.0;
    
    void addSample(double v) {
        samples.push_back(v);
        if (samples.size() > 300) samples.erase(samples.begin());
        std::vector<double> tmp = samples;
        std::sort(tmp.begin(), tmp.end());
        if (!tmp.empty()) {
            p50 = tmp[tmp.size() / 2];
            size_t idx95 = static_cast<size_t>(std::ceil(tmp.size() * 0.95)) - 1;
            if (idx95 >= tmp.size()) idx95 = tmp.size() - 1;
            p95 = tmp[idx95];
        }
        spike = (v > spike_threshold_ms);
        if (v > hard_kill_ms) halted = true;
        if (halted && p50 < 10.0) halted = false;
    }
};
static LatencyTracker g_latency;

void push_latency_to_gui()
{
    ...
    g_telemetry.Update(
        xau_bid, xau_ask, xag_bid, xag_ask,
        g_dailyPnL, 0.0,
        g_latency.last_net_rtt,
        g_latency.p50,
        g_latency.p95,
        g_latency.spike ? "SPIKE" : "NORMAL",
        g_latency.halted ? "HALTED" : "CONNECTED",
        "NONE", "NONE"
    );
}
...
bool load_config(const std::string& path)
{
    ...
    while (std::getline(f, line)) {
        ...
        if (key == "mode") {
            if (val == "LIVE") g_mode = TradingMode::LIVE;
            else if (val == "SAFE") g_mode = TradingMode::SAFE;
            else g_mode = TradingMode::SHADOW;
        }
    }
...

{
    ...
    while (session.isRunning()) {
        ...
        while (session.isRunning()) {
            // Active latency ping every 5 seconds
            static auto last_ping = std::chrono::steady_clock::now();
            auto now_ping = std::chrono::steady_clock::now();
            if (std::chrono::duration_cast<std::chrono::seconds>(now_ping - last_ping).count() >= 5) {
                std::string testId = "PING-" + std::to_string(session.nextSeq());
                int seq_ping = session.nextSeq();
                std::stringstream ping_body;
                ping_body << "35=1\x01"
                         << "49=" << g_cfg.sender << "\x01"
                         << "56=" << g_cfg.target << "\x01"
                         << "50=QUOTE\x01"
                         ...
                         << "52=" << timestamp() << "\x01"
                         << "112=" << testId << "\x01";
                std::string ping_msg = wrap_fix(ping_body.str());
                if (session.sslWrite(ping_msg.c_str(), static_cast<int>(ping_msg.size()))) {
                    g_latency.pendingPings[testId] = std::chrono::high_resolution_clock::now();
                }
                last_ping = now_ping;
            }
            ...
            for (const auto& msg : messages) {
                ...
                if (msg.find("35=W") != std::string::npos || msg.find("35=X") != std::string::npos) {
                    try {
                        ...
                        g_telemetry.Update(xau_bid, xau_ask, xag_bid, xag_ask, 0.0, 0.0, g_latency.last_net_rtt, g_latency.p50, g_latency.p95, "NORMAL", "CONNECTED", "NONE", "NONE");
                        ...
                        if (symbolId == 41 && bid > 0 && ask > 0) {
                            ...
                            if (!g_audit.allowTrading()) {
                                std::cout << "[BLOCKED] Audit layer\n";
                                continue;
                            }
                            
                            if (g_staleGuard.isStale()) {
                                std::cout << "[BLOCKED] Stale tick\n";
                                g_audit.recordStale();
                                continue;
                            } else {
                                g_audit.clearStale();
                ...
                auto hftIntent = [&]() -> AlphaIntent {
                    ...
                    if (!g_hftFilter.allow(state.sweep_size, 0.3, state.spread, true)) {
                        std::cout << "[HFT] Filter rejected: sweep=" << state.sweep_size << " spread=" << state.spread << "\n";
                        return intent;
                    }
                    if (state.sweep_detected) {
                        ...
                        std::cout << "[HFT] Intent VALID: long=" << intent.long_signal << " short=" << intent.short_signal << "\n";
                    }
                    return intent;
                };
                ...
                auto structureIntent = [&]() -> AlphaIntent {
                    ...
                    if (g_structure.breakoutLong()) {
                        ...
                        std::cout << "[STRUCTURE] Intent VALID: LONG\n";
                    } else if (g_structure.breakoutShort()) {
                        intent.valid = true;
                        intent.short_signal = true;
                        intent.confidence = 0.85;
                        std::cout << "[STRUCTURE] Intent VALID: SHORT\n";
                    }
                    return intent;
                };
                
                // Diagnostic state
                std::cout << "[STATE] mode=" << (g_mode == TradingMode::LIVE ? "LIVE" : g_mode == TradingMode::SHADOW ? "SHADOW" : "SAFE")
                          << " sweep=" << state.sweep_detected 
                          << " compression=" << state.compression
                          << " drawdown=" << state.drawdown_active
                          << " latency_halt=" << g_latency.halted << "\n";
                ...
                g_prod.update(
                    ...
                    [&](bool isLong, double size) {
                        // Execute callback
                        std::cout << "[ENGINE] Execute callback ENTERED: isLong=" << isLong << " size=" << size << "\n";
                        ...
                        if (g_latency.halted) {
                            std::cout << "[LATENCY] HARD HALT\n";
                            return;
                        }
                        
                        if (g_mode == TradingMode::SAFE) { std::cout << "[SAFE] Suppressed\n"; return; }
                        if (g_mode == TradingMode::SHADOW) { std::cout << "[SHADOW] " << (isLong ? "BUY" : "SELL") << " " << size << "\n"; return; }
                        ...
                        std::string side = isLong ? "1" : "2";  // 1=Buy, 2=Sell
                                ...
                                g_latency.pendingOrders[clOrdId] = std::chrono::high_resolution_clock::now();
                ...
                if (msg.find("35=1") != std::string::npos) {
                    ...
                    if (!tid.empty()) {
                        ...
                        // Measure network RTT
                        auto it = g_latency.pendingPings.find(tid);
                        if (it != g_latency.pendingPings.end()) {
                            auto now = std::chrono::high_resolution_clock::now();
                            double net_rtt = std::chrono::duration<double, std::milli>(now - it->second).count();
                            g_latency.last_net_rtt = net_rtt;
                            g_latency.addSample(net_rtt);
                            g_latency.pendingPings.erase(it);
                            push_latency_to_gui();
                        }
            ...
            for (const auto& msg : messages) {
                ...
                if (msg.find("35=8") != std::string::npos) {
                    try {
                        ...
                        // Measure execution RTT
                        auto it = g_latency.pendingOrders.find(clOrdId);
                        if (it != g_latency.pendingOrders.end()) {
                            auto now = std::chrono::high_resolution_clock::now();
                            double exec_rtt = std::chrono::duration<double, std::milli>(now - it->second).count();
                            g_latency.last_exec_rtt = exec_rtt;
                            g_latency.addSample(exec_rtt);
                            g_latency.pendingOrders.erase(it);
                            push_latency_to_gui();
                        }
